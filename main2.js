...
async function getStreams() {
    ...
    const userResponse = await fetch(`https://api.twitch.tv/helix/users?login=${channels.join('&login=')}`, {
      headers: {
        'Client-ID': twitchClientId,
        'Authorization': `Bearer ${await getAccessToken()}`
      }
    ...
    const offlineChannels = channels.filter(channel => !streamData.data.some(stream => stream.user_name === channel));
    
    const offlineUsers = userData.data.filter(user => offlineChannels.includes(user.login));
    
    const offlineStreams = offlineUsers.map(user => {
      return {
    ...
    return streamData.data.concat(offlineStreams);
  }
